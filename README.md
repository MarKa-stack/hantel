...
- **Fortschritt**: Übersicht mit Kennzahlen-Fenster zum Blättern (Diese Woche mit Wochentagen, Workouts gesamt, Serie mit 8-Wochen-Leiste), Karten **Dein Coach** (s.u.), Erholung (→ Muskelgruppen), Gewicht & Maße, Rekorde & Meilensteine (eine Seite, zwei Reiter), Verlauf (Kalender/Wochen-Chart + letzte Workouts) einklappbar mit gemerktem Zustand; je Übung Kennzahlen (Bestes Gewicht, Top-Satz, e1RM, Volumen, Wdh) × Zeiträume (1M/3M/6M/1J/Gesamt) mit Tooltip (Datum, Gewicht × Wdh, Satz, e1RM), Veränderung („+13 kg in 5 Monaten“), Statistik und Rekorde
...
- **Rund ums Training**: „Heute dran“-Karte mit A/B-Rotation, Gewicht & Maße mit Verlauf, **Fortschrittsfotos** (`#/photos`: Pose Vorne/Seite/Hinten, Datum, Gewicht aus dem Körperlog; Galerie nach Monat; Vorher/Nachher-Vergleich mit Schieberegler; Bilder liegen als JPEG max. 1200 px + Thumbnail in IndexedDB, nur lokal – nicht in Sicherung/Cloud, `js/photos.js`), Trainingskalender (Heatmap), vergangene Sessions korrigierbar, 1RM-Prozent-Tabelle je Übung
...
- **Dein Coach** (`#/coach`): wöchentlicher KI-Bericht – `js/coach.js` verdichtet die letzten vier Wochen zu Kennzahlen (Trainings/Sätze/Volumen/Rekorde je Woche, letzte 7 Tage im Detail, Übungen mit Arbeitsgewicht, e1RM-Trend und Stagnation, Sätze je Muskelgruppe gegen den Zielbereich, müde Muskeln, Gewichtstrend, Kalorien-/Proteinschnitt; ~2–4 KB JSON, keine Rohdaten oder Fotos) → Aufgabe `coach` (Schema: headline, grade top/gut/ok/schwach, wins, watch, next[{title, detail}], focus) → Seite mit Note, Kernsatz, Fokus, „Was lief gut“, „Darauf achten“, „Nächste Woche“; Bericht wird in den Einstellungen gemerkt (`settings.coach`), Karte auf Fortschritt zeigt Kernsatz und „Neu“, sobald eine neue Woche mit Training begonnen hat; Erstellen per Tipp (nie automatisch)
- **Meilensteine**: 23 aus dem Verlauf berechnete Marken (Workouts, Serien, Volumen, Rekorde, 100 kg …), Toast nach dem Workout, Reiter auf `#/records` (Rekorde & Meilensteine; `#/milestones` öffnet den Reiter direkt) (`js/milestones.js`)
...
- **Mehr**: Übungsbibliothek, Plan-Vorlagen, Gewicht & Maße als Karten; **Akzentfarbe** wählbar (Orange, Blau, Violett, Grün, Türkis, Pink – `js/theme.js` setzt `--accent`, `--accent-2`, `--accent-soft`, `--accent-rgb` auf `<html>`, Hell/Dunkel je eigene Abstufung, auch auf der Share-Karte); die Einstellungsbereiche (Darstellung, Training, KI, Daten, Cloud-Backup, Trainingstage, App installieren) sind einklappbare Karten mit Kurzinfo in der Unterzeile, Zustand wird gemerkt
...
js/theme.js           Akzentfarben
js/coach.js           Wöchentlicher KI-Coach (Datenverdichtung + Bericht)
js/photos.js          Fortschrittsfotos (IndexedDB)
