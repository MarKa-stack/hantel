...
- **Design (1.11)**: flache Flächen ohne Rahmen/Verläufe, Startseite mit „Heute dran“-Karte (Plan-Muskelfokus, Chips, Start-Button), Erholung/Essen/Woche als Icon-Zeilen, Plan-Karten mit Direktstart (Play), Profil-Knopf oben rechts; Plan-Optionen (⋯) in der Plan-Ansicht. Dashboard mit Wochenring/Serie/letztem PR, Display-Schrift (Space Grotesk) für Titel und Zahlen, eigenes SVG-Icon-Set, Seitenübergänge und Mikro-Animationen (Haken zeichnet sich, Einrasten, Count-up, aufleuchtende Muskelkarte; `prefers-reduced-motion` wird respektiert), fokussierter Workout-Screen (kompakter Kopf, nur der aktuelle Satz groß, Pausenring inline, Auto-Scroll zum Ring), Begrüßung mit Namen (unter „Mehr“), iOS-Splash-Screens, Haptik über switch-Checkbox (iOS 17.4+)
